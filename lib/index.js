    ...
    if (options['global-config']) {
        ...
        find(process.cwd(), '.grifter.json', function(err, file) {
            if (file) {
                ...
                find(process.cwd(), '.yogi.json', function(err, file) {
                    if (file) {
                        log.info('woohoo, found a yogi config here: ' + file);
                        var json = JSON.parse(fs.readFileSync(file, 'utf8'));
                        ['username', 'token', 'gh_user', 'gh_token'].forEach(function(k) {
                            if (!options[k]) {
                                log.info('[yogi] override config found for ' + k);
                                options[k] = json[k];
                            }
                        });
                    }
